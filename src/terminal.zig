...
pub fn editorReadKey() !u16 {
    ...
    buf[0] = 0;
    var n: usize = try stdinread.read(buf[0..]);
    while (n == 0) n = try stdinread.read(buf[0..]);
    if (n == -1) {
        return termError.read;
    }
    if (buf[0] == '\x1b') {
...
pub fn getWindowSize(rows: *usize, cols: *usize) !void {
    ...
    switch (posix.errno(os.linux.ioctl(os.linux.STDOUT_FILENO, os.linux.T.IOCGWINSZ, @intFromPtr(&ws)))) {
        os.linux.E.SUCCESS => {
            rows.* = ws.ws_row;
            cols.* = ws.ws_col;
        },
